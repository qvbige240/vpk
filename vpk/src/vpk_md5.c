...
//const HMAC_params Vpk_HMAC_MD5[] = {
//  {
...
const vpk_md5_t VPK_DIGEST_MD5[] = {
  {
    (VpkMD5Init) MD5_Init,			/* Digest initialization function */
    (VpkMD5Update) MD5_Update,		/* Digest update function */
    (VpkMD5Final) MD5_Final,		/* Digest computation end function */
    sizeof(MD5_CTX),				/* Size of digest context struct */
    16								/* Result size */
  }
};

void vpk_md5_bgen(unsigned char *outbuffer, /* 16 bytes */ 
				 const unsigned char *input)
{
	MD5_CTX ctx;
	MD5_Init(&ctx);
...
vpk_md5ctx_t *vpk_md5_init(const vpk_md5_t *md5params)
{
	vpk_md5ctx_t *ctx;

	/* Create MD5 context */
	ctx = malloc((sizeof *ctx) + (md5params->md5_ctxtsize));
	...
	//ctx->md5_ctx = malloc(md5params->md5_ctxtsize);
	ctx->md5_ctx = (void *)ctx + (sizeof *ctx);
	...
	ctx->md5_hash = md5params;

	(*md5params->md5_init)(ctx->md5_ctx);

	return ctx;
}

int vpk_md5_update(vpk_md5ctx_t *context, const unsigned char *data, unsigned int len)
{
	(*context->md5_hash->md5_update)(context->md5_ctx, data, len);

	return 0;
}

int vpk_md5_final(vpk_md5ctx_t *context, unsigned char *result)
{
	(*context->md5_hash->md5_final)(result, context->md5_ctx);
...
int hash_dataf(MD5_CTX* ctx, FILE *input)
{
	FILE* fp = input;
	...
	while (offset < file_size) {
		...
		MD5_Update(ctx, (const unsigned char *) data, ret);
...
int hash_data_file(MD5_CTX* ctx, const char* filename)
{
	int ret = 0;
	FILE* fp = 0;
	return_val_if_fail(filename != NULL, -1);
	...
	fp = fopen(filename, "r");
	ret = hash_dataf(ctx, fp);
	fclose(fp);

	return ret;
}

int gen_md5(const char* filename, unsigned char* hash_hex)
{
	int i = 0;
	MD5_CTX ctx;
...
void vpk_md5_fgen(unsigned char *output, FILE *input)
{

}

void vpk_md5_file_gen(unsigned char *output, const char *path)
{
