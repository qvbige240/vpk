...
static int nt966x_serial_opt_set(vpk_session_t *session);
...
static int nt966x_serial_destruct(void *session)
{
...
static vpk_session_t* nt966x_serial_load(void *tty)
{
	...
	if (thiz)
	{
		thiz->destruct   = (vpk_destruct_func)nt966x_serial_destruct;
...
static int nt966x_serial_open(vpk_session_t *session, void *tty)
{
	int ret = -1;
	vpk_serial_t* thiz = (vpk_serial_t*)session;
...
//
static int nt966x_serial_read(vpk_session_t *session, void *buf, size_t nbytes, int timout, void* visit)
{
...
static int nt966x_serial_write(vpk_session_t *session, void *buf, size_t nbytes, int timout)
{
...
static int nt966x_serial_close(vpk_session_t *session)
{
	vpk_serial_t* thiz = (vpk_serial_t*)session;
	if (thiz && thiz->fd >= 0)
	{
...
static const session_ops nt966x_serial_ops =
{
	nt966x_serial_load,
	nt966x_serial_open,
	nt966x_serial_read,
